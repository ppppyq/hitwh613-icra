...
cap_2 = cv.VideoCapture()  # 在这个括号里面填入第二个摄像头的id
cap_2.set(3, 448)
cap_2.set(4, 404)
...
while True:
    ...
    lower = np.array([[60, 0, 17],
                      [44, 0, 122]])  # 车
    upper = np.array([[160, 90, 137],
                      [144, 84, 242]])  # 车
    ...
    for k in range(2):
        ...
        for cnt in contours:
            ...
            if area > 2000 and w > 30 and h > 30:
                ...
                mask_blue = cv.inRange(judge, lower[1], upper[1])


                area_blue = judgeC(mask_blue)
                if k == 0:
                    if area_blue > 500:
                        cv.putText(original_img[k], f'blue ({(448-(x + w)) / 200},{(y + h) / 200})', (x, y - 3),
                               cv.FONT_HERSHEY_DUPLEX, 0.5, (0, 0, 255), 1)
                        cv.rectangle(original_img[k], (x, y), (x + w, y + h), (0, 0, 255), 1)
                        print(adress[k])  # 打印车中心坐标
                if k == 1:
                    if area_blue > 500:
                        cv.putText(original_img[k], f'blue ({(448-(x + w)) / 200},{(y + h + 404) / 200})', (x, y - 3),
                               cv.FONT_HERSHEY_DUPLEX, 0.5, (0, 0, 255), 1)
                        cv.rectangle(original_img[k], (x, y), (x + w, y + h), (0, 0, 255), 1)
                        print(adress[k])  # 打印车中心坐标
